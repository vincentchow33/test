		...
		if((st+wh)>(ot+oh)){
			if(execute){
				...
				if(info.data){
					...
					info.data = info.data.replace(/\&initials\=\d+/g, '');
					info.data = info.data.replace(/\&search\=\d+/g, '');
					info.data += '&cat_id='+cat_id+'&have_num='+have_num+'&load_num='+load_num+'&initials='+initials+'&search='+search;
				}
...
function readyLoad(){
	...
	if(liftObj.data("type") == "one"){
		returnTop = '<div class="lift-item lift-item-top" ectype="liftItem"><i class="iconfont icon-up"></i></div>';
	}else{
