...
$(function(){
	...
	$(document).on("click","*[ectype='coll_brand']",function(){
		...
		if(user_id > 0){
			...
			if($(this).find("i").hasClass("icon-zan-alts")){
				$(this).closest("li").removeClass("followed");
				...
				$(this).closest("li").addClass("followed");
	...
	$(document).on("click", "*[ectype='brandCate'] *[ectype='cateItem']", function(){
		...
		var initials = $("*[ectype='brandInitials'] a.curr").data("initials"); //获取首字母（自定义部分，可修改）
		var search = $("#brandSearch").val();
		
		$(this).addClass('curr').siblings("*[ectype='cateItem']").removeClass('curr');
		
		$.jqueryAjax('brand.php', 'act=filter_category&cat_id='+cat_id+'&initials='+initials+'&search='+search, function(data){
			$("*[ectype='brandList'] *[ectype='items']").html(data.content);
			$("#paging").html(data.paging);
			$("#brand-list-exclusive *[ectype='items']").html(data.exclusive);
		});
	});
	...
	//品牌专区首页分类筛选
	$(document).on("click", "*[ectype='brandInitials'] *[ectype='initialsItem']", function(){
		$(this).addClass('curr').siblings("*[ectype='initialsItem']").removeClass('curr');
		var initials = $("*[ectype='brandInitials'] a.curr").data("initials"); //获取首字母（自定义部分，可修改）
	...
	//收货人信息切换
	$edit_consignee_checked_loading = false;

	$(document).on("click","*[ectype='cs-w-item']",function(e){

		if (e.target.className == 'edit' || e.target.className == 'delete') {
			return; // return if Modify or Delete button is clicked because it will trigger Order total calculation which is not required
		}

		if ($edit_consignee_checked_loading) {
			alert('Please wait while shipping is calculated');
			return;
		}
		$edit_consignee_checked_loading = true;

		add_checkout_msg('Please wait while shipping fee is calculated');
		...
		Ajax.call('flow.php?step=edit_consignee_checked', 'address_id=' + address_id + '&store_id=' +store_id + '&uc_id=' + uc_id + '&shipping_id=' + $.toJSON(shipping_id), function (result) {
			if(result.error > 0){
				if(result.error == 1){
					...
					add_checkout_msg(result.msg);
					alert(result.msg);
					return false;
				}
			}else{
				if ('shipping_err_code' in result.total && result.total.shipping_err_code != 0) {
					$('#submit-done').prop('disabled', true);
				} else {
					$('#submit-done').prop('disabled', false);
				}
	...
	$(document).on("click","*[ectype='dialog_checkout']",function(){
		...
		if(obj.divId == 'new_address' || obj.divId == 'edit_address'){
			...
			if ($('#address_remove').length > 0) {
				$('#address_remove').show();
				$('#address_remove #confirm-delete-address').attr('data-id', obj.id);
			} else {
...
//批发-发票修改
	...
	$("*[ectype='bankList'] li").on("click",function(){
		...
		if(parent.find(".selected").length>0){
			...
			$("#alipay_bank").find("input").show().css({"background-color":"#C11B17"});
		}
	});
...
function get_toCart(){
	...
	if(num == 0){
		...
		if ($('#flow_add_cart_custom').length > 0) {
			$('#flow_add_cart_custom').show();
			$("#flow_add_cart_custom .message").html('Please select at least one item.');
		} else {
			pb({
				id:"flow_add_cart",
				title:json_languages.pb_title,
...
function addUpdate_Consignee(frm){
	...
	csg.location_lat 		= frm.find("[name='consignee_location_lat']").val();
	csg.location_lng 		= frm.find("[name='consignee_location_lng']").val();
	...
	$('#consignee_location-error').remove();
	$('#mapContainer').css('border', 'none');
	if (!$('#consignee_location_lat').val() || !$('#consignee_location_lng').val()) {
		$('#mapContainer').after('<label id="consignee_location-error" style="color: #C11B17;" for="consignee_location">Address location is required</label>');
		$('#mapContainer').css('border', '1px solid #C11B17');
		location_valid = false;
	} 

	if(frm.valid() && location_valid){
...
$(document).on('click', '#search_history_list li:not(.title)', function() {
	let keyword = $(this).text();
	let search_text = $(this).closest('.search_history_wrap').siblings('.search-text');
	search_text.val(keyword);
	search_text.siblings('button:first').click();
});

let search_bar = $('.header .search-text');
if (search_bar.length > 0) {
	search_bar.after($('<div>', {
		class: 'search_history_wrap',
		html: $('<ul>', { id: 'search_history_list' }),
		style: 'width: ' + search_bar.outerWidth() + 'px;',
	}));

	// get recent_searches cookies from browser
	var cookieValue = null;
	if (document.cookie && document.cookie != '') {
		var cookies = document.cookie.split(';');
		for (var i = 0; i < cookies.length; i++) {
			var cookie = cookies[i].trim();
			if (cookie.substring(0, 16) == 'recent_searches=') {
				cookieValue = cookie.substring(16, cookie.length);
				break;
			}
		}
	}
	...
	if (cookieValue) {
		// decode cookie value
		let recent_searches_arr = decodeURIComponent(cookieValue.replace(/\+/g, ' '));
		recent_searches_arr = JSON.parse(recent_searches_arr);

		// Get the unordered list element from the DOM
		var $ul = $("#search_history_list");
		$ul.append($('<li>', { html: '<span>Recent Searches</span><div class="line-deco"></div>', class: 'title' }));

		// Loop over the JSON array and create list items
		$.each(recent_searches_arr, function(index, value) {
			var $li = $("<li>");
			$li.text(value);
			$ul.append($li);
		});
...
jQuery.inputPrompt = function(s,c,v){
	var s = $(s);
	s.keyup(function(){
		$(this).siblings('.search_history_wrap').hide();
	});
	s.focus(function(){
		if($(this).val() == v){
			var url_string = window.location.href; 
			var url = new URL(url_string);
			var encode = url.searchParams.get("encode");

			if (!url_string.includes("/search.php") || !encode) {
				$(this).val("");
			}
		...
		if ($(this).siblings('.search_history_wrap').find('li').length > 0 && !$("#suggestions").is(":visible")) {
			$(this).siblings('.search_history_wrap').show();
		}
	});
	s.blur(function(){
		if($(this).val()==''){
			...
			if(data.type == 1){
				if(type == 'goods'){
					$("[ectype='collect_store']").each(function(){
						...
						obj.html("<i class='iconfont icon-zan-alt'></i>");
					});
				}else if(type == 'store'){
					obj.removeClass("selected");
					obj.html("<i class='iconfont icon-zan-alt'></i>");
				...
				if(type == 'goods'){
					$("[ectype='collect_store']").each(function(){
						...
						obj.html("<i class='iconfont icon-zan-alts'></i>");
					})
				}else if(type == 'store'){
					obj.addClass("selected");
					obj.html("<i class='iconfont icon-zan-alts'></i>");
